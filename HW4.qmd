...
species_map <- tm_shape(depth_resampled) +
  ...
  tm_raster(palette = rev(hcl.colors(5, "BuGn")),
            style = "cat",
            title = "Total suitable area (km) / EEZ region",
            labels = c("Lowest, 438",
                       "Low, 1,533",
                       "Mid, 3,225",
                       "High, 4,097",
  ...
  tm_layout(fontfamily = "courier",
            ...
            legend.width = 7,  # Adjust width if needed
            legend.height = 0.5) +
  tm_graticules(col = "white",
                lwd = 0.3,
                alpha = 0.5) 

species_map <- tmap_save(species_map, "species_map.html", width = 1200, height = 800)
...
```{r}
# suitable_locations set to NAs
# then mask suitable cells, using eez as vector 
# cell area of suitable locations 
# eez_rast <- rasterize(eez), can use suitable locations rast as template
# need template raster with matching res, crs 
# zonal function terra
#  
# field argument rasterize()
# 
# any species, megaladon 
```
